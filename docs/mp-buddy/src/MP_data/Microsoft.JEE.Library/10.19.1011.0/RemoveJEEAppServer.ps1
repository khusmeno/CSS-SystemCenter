##########################################################################################
# <copyright file="RemoveJEEAppServer.ps1" company="Microsoft">
#     Copyright (c) Microsoft Corporation.  All rights reserved.
# </copyright>
# <summary>Powershell script to remove a first stage discovery of a Java JEE 
# application server. A "Microsoft.JEE.Universal.ApplicationServer" object. 
...
begin 
{
    ...
    #
    # If all the requirements are met the matching Operations Manager object is removed.
    # 
    ...
    {
        #
        # Validate that the input URL is correctly formed, the port and hostname are
		...
		$jeeAppServers = $managementGroup.GetMonitoringObjects($jeeAppServerClass)
	    $baseAppServerClass = GetMonitoringClass $managementGroup "Microsoft.JEE.ApplicationServer.Instance"

		$id = "$($appServerHost):$($appServerPort)"

		foreach ($server in $jeeAppServers)
		{
			$serverid = $server.GetMonitoringPropertyValue($baseAppServerClass.GetMonitoringProperty("Id"))
			if ($serverid.StartsWith($id) -and $serverid.EndsWith(":universal"))
			{
				$monitoringConnector = GetJEEConnector $managementGroup
				$discoveryData = new-object Microsoft.EnterpriseManagement.ConnectorFramework.IncrementalMonitoringDiscoveryData
			    $discoveryData.Remove($server)
				$discoveryData.Commit($monitoringConnector)
		        Write-Host -noNewLine  "Removed app server: "
		        Write-Host -foregroundcolor Green $jeeAppServer
				break
			}
		}
    }
    ...
    if ($Help)
    {
        $helpstr =  "`n`nRemove-JEEAppServer.ps1`n" +
                    "`n" +
                    "`nRemoves JEE App Servers from Operations Manager.`n" +
                    "`n" +
                    "Input:`n" +
                    ...
                    "`n" +
                    "Output:`n" +
                    "  The script displays an error message for each app server it fails to remove.`n" +
                    "`n" +
                    "Parameters:`n" +
                    ...
                    "`n" +
                    "Examples:`n" +
                    "  Remove-JEEAppServer.ps1 -Target http://www.contoso.com:8080`n" +
                    "  type c:\MyAppServers.txt | Remove-JEEAppServer.ps1`n" +
                    "`n"
        Write-Host $helpstr
        exit
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU0zbNrPTZZgCKG06hJdY/EiiI
# zyugghXyMIIEoDCCA4igAwIBAgIKYRr16gAAAAAAajANBgkqhkiG9w0BAQUFADB5
...
# BDEWBBSPdaWJUjjMGcs5RgkiIdAto5he7zCBgAYKKwYBBAGCNwIBDDFyMHCgRIBC
...
# eXN0ZW1DZW50ZXIgMA0GCSqGSIb3DQEBAQUABIIBAKaxbKG4GRR//CVNKBltpSUL
# 5P3E0LYkez75Y/Lt1+N6B056sKdJSq1Em8UvtaYIaDG13IrDjyqqWZYQD90eIUqj
# l23iZ304iC4RbJxHNjuZIx6CtqKr28zpVJHzoALM6nUmn43XvGdmNuORBi0aGECo
# L9lJzZqJkKZVYS6jS+f4F3ZIDdWAAndYewxqu9WSm3/Rr8c+guRM6yNmtoIHxhA6
# 6gIN9d3gyJxINv4VpvOtQovGXRhbjnBF+6fUntFbabLjpaGfmHpFELkeTKoydKix
# u93V4O5F52r45fhtdRQQlBMzK07Lz78r46uWfeVXUNkWM8cWWHuZg7LyvicKMQ2h
...
# 9w0BCQQxFgQUm8OaDUFhfeveOZiLQBCwATvfA5UwDQYJKoZIhvcNAQEFBQAEggEA
# yJq+K55mXSng5pse8fHzFJSp3ZDG+ANzFnRJ3WNipHqDyrlcOECW9w6SHlnxcNdt
# YHrCbm8vmZVsMFfUdJ15u7SxU2uTr4uz8Ry6qxLT/N13Z02JRSfAdVfNm4oYd1a0
# d1tlDoyRoVR3YJ2mGNIxcp/uUekEiqxVH0+JfgUPWs5bisOkDdwFLCqhWp5GqBEU
# beN4KH99aqIqVmYJJYPflsWDAHFzvZLtfnk8iSVxrtKz7KOCyT/bwam3lceZb7rP
# HruKVkYoM29cxsZVExWX7FcF4Gi6oBRSEArHxZykxtdDFGasVvC9Qj5VGzO9ToYo
# qKRUZYJgLKTZ5ORGagnFmw==
