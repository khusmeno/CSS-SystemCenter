        ...
        Array.from(nodes).map(async (node) => {
            ...
            for (const attr of Array.from(allAttributes).filter(attr => attr !== 'ID')) {
                ...
                if (isRefElem) {
                    ...
                    if (value.includes('!')) {
                        cellContent = `<a title="Click to fix" href="https://github.com/microsoft/CSS-SystemCenter/blob/main/docs/mp-buddy/README.md#how-to-add-missing-links-to-other-elements" target="_blank" rel="noopener">?? </a>${value}`
                    }
